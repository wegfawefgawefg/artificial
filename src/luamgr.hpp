...
struct ItemDef {
  ...
  std::string sound_use; std::string sound_pickup;
...
struct GunDef {
  std::string name; int type=0; float damage=0.0f; float rpm=0.0f; float recoil=0.0f; float control=0.0f; int mag=0; int ammo_max=0; std::string sprite;
  std::string sound_fire; std::string sound_reload; std::string sound_jam; std::string sound_pickup;
...
struct ProjectileDef {
  std::string name; int type=0; float speed=20.0f; float size_x=0.2f, size_y=0.2f; int physics_steps=2;
...
struct CrateDef {
  std::string name; int type=0; float open_time=5.0f; std::string label;
  DropTables drops;
  int on_open_ref{-1};
};
...
public:
  ...
  const ProjectileDef* find_projectile(int type) const { for (auto const& p : projectiles_) if (p.type==type) return &p; return nullptr; }
  const GunDef* find_gun(int type) const { for (auto const& g : guns_) if (g.type==type) return &g; return nullptr; }
  ...
  const CrateDef* find_crate(int type) const { for (auto const& c : crates_) if (c.type==type) return &c; return nullptr; }
  ...
  const std::vector<ProjectileDef>& projectiles() const { return projectiles_; }
  const DropTables& drops() const { return drops_; }
  const std::vector<CrateDef>& crates() const { return crates_; }
  ...
  void add_projectile(const ProjectileDef& d) { projectiles_.push_back(d); }
  void add_crate(const CrateDef& d) { crates_.push_back(d); }
