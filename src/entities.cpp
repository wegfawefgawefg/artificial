...
std::optional<VID> Entities::new_entity() {
  if (free_ids.empty()) return std::nullopt;
...
void Entities::set_inactive_vid(VID vid) {
  ...
  if (e.active && e.vid.version == vid.version) set_inactive(vid.id);
}

Entity* Entities::get_mut(VID vid) {
  Entity& e = items[vid.id];
  if (e.active && e.vid.version == vid.version) return &e;
  return nullptr;
}
...
std::vector<VID> Entities::active_vids() const {
  ...
  for (auto const& e : items) if (e.active) out.push_back(e.vid);
  return out;
}
