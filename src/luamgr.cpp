...
bool LuaManager::register_api() {
  ...
  s.set_function("register_item", [this](sol::table t){
    ItemDef d{}; d.name=t.get_or("name", std::string{}); d.type=t.get_or("type", 0); d.category=t.get_or("category", 0); d.max_count=t.get_or("max_count", 1); d.consume_on_use=t.get_or("consume_on_use", false); d.sprite=t.get_or("sprite", std::string{}); d.desc=t.get_or("desc", std::string{});
  ...
  s.set_function("register_gun", [this](sol::table t){
    GunDef d{}; d.name=t.get_or("name", std::string{}); d.type=t.get_or("type", 0); d.damage=t.get_or("damage", 0.0f); d.rpm=t.get_or("rpm", 0.0f); d.recoil=t.get_or("recoil", 0.0f); d.control=t.get_or("control", 0.0f); d.mag=t.get_or("mag", 0); d.ammo_max=t.get_or("ammo_max", 0); add_gun(d);
  });
