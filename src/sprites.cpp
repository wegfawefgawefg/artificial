...
const SpriteDef* SpriteStore::get_def_by_id(int id) const {
  ...
  if (idx >= defs_by_id.size()) return nullptr;
...
static bool parse_bool(const std::string& v, bool& out) {
  if (v == "true" || v == "True" || v == "TRUE") { out = true; return true; }
  if (v == "false" || v == "False" || v == "FALSE") { out = false; return true; }
  return false;
}

static bool parse_int(const std::string& v, int& out) {
  try {
    size_t p=0; long val = std::stol(v, &p, 10);
...
static bool parse_float(const std::string& v, float& out) {
  try {
    size_t p=0; double val = std::stod(v, &p);
    if (p != v.size()) return false;
...
SpriteDef make_default_sprite_from_image(const std::string& name, const std::string& image_path) {
  ...
  SpriteFrame f{}; f.x = 0; f.y = 0; f.w = 0; f.h = 0; f.duration_sec = 0.0f;
  d.frames.push_back(f);
  return d;
}

bool parse_sprite_manifest_file(const std::string& path, SpriteDef& out_def, std::string& out_error) {
  std::ifstream f(path);
  if (!f.good()) { out_error = "failed to open"; return false; }
  SpriteDef def{};
  // Defaults
  def.loop = false; def.fps = 0.0f; def.pivot_px_x = -1; def.pivot_px_y = -1; def.world_offset_x = 0.0f; def.world_offset_y = 0.0f;
  ...
  while (std::getline(f, line)) {
    ...
    if (!in_frames && t.rfind("frames", 0) == 0) {
      ...
      if (rhs.size() && rhs[0]=='[' && rhs.back()==']') {
        ...
        for (const auto& it : items) {
          ...
          SpriteFrame fr{};
          if (!parse_int(parts[0], fr.x)) continue;
          if (!parse_int(parts[1], fr.y)) continue;
          if (!parse_int(parts[2], fr.w)) continue;
          if (!parse_int(parts[3], fr.h)) continue;
          if (parts.size() >= 5) { (void)parse_float(parts[4], fr.duration_sec); }
    ...
    if (key == "name") {
      ...
      int a= -1, b= -1; if (parse_int_pair(val, a, b)) { def.pivot_px_x = a; def.pivot_px_y = b; }
    } else if (key == "world_offset") {
      std::string s = val; if (s.size()<2 || s.front()!='[' || s.back()!=']') { /* ignore */ }
      else {
        std::string inner = trim(s.substr(1, s.size()-2));
        auto parts = split_csv(inner);
        if (parts.size()==2) {
      ...
      if (val.size()<2 || val.front()!='{' || val.back()!='}') continue;
      ...
      for (const auto& kvs_item : kvs) {
        auto eq2 = kvs_item.find('='); if (eq2==std::string::npos) continue;
        std::string k = trim(kvs_item.substr(0, eq2));
        std::string v = trim(kvs_item.substr(eq2+1));
        (void)parse_int(v, fw); (void)parse_int(v, fh); // to avoid -Wunused warnings when not matched
        if (k=="frame_w") (void)parse_int(v, fw);
        else if (k=="frame_h") (void)parse_int(v, fh);
        else if (k=="cols") (void)parse_int(v, cols);
        else if (k=="rows") (void)parse_int(v, rows);
        else if (k=="origin_x") (void)parse_int(v, ox);
        else if (k=="origin_y") (void)parse_int(v, oy);
      }
      if (fw>0 && fh>0 && cols>0 && rows>0) {
        ...
        for (int r=0; r<rows; ++r) {
          for (int c=0; c<cols; ++c) {
            SpriteFrame fr{}; fr.x = ox + c*fw; fr.y = oy + r*fh; fr.w = fw; fr.h = fh; fr.duration_sec = 0.0f;
            frames_tmp.push_back(fr);
          }
        }
      }
  ...
  if (!frames_tmp.empty()) def.frames = std::move(frames_tmp);
  if (def.frames.empty()) {
    // Default to whole image placeholder
    SpriteFrame fr{}; def.frames.push_back(fr);
  }
