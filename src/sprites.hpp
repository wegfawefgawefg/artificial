...
struct SpriteIdRegistry {
  ...
  int size() const { return static_cast<int>(name_to_id.size()); }
  // Returns a stable vector of names ordered by ID (for debugging/UI).
  const std::vector<std::string>& names_by_id() const { return id_to_name; }
...
public:
  ...
  int size() const { return static_cast<int>(defs_by_id.size()); }
  const std::vector<SpriteDef>& defs() const { return defs_by_id; }
